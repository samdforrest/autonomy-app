...
export default function CuriosityParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Curiosity Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Curiosity Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Curiosity Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Curiosity Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Curiosity Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Curiosity Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Curiosity analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Curiosity Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Curiosity Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Curiosity Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Curiosity Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Curiosity Parsed content length:', parsedContent.length);
    console.log('🔍 Curiosity Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Curiosity Parent Guide</ThemedText>
        <ThemedText style={styles.moduleSubtitle}>
          Fostering questions and exploration in your child
        </ThemedText>
      </ThemedView>
      ...
      <SurveyButton 
        moduleId="curiosity-parent"
        moduleName="Curiosity Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
