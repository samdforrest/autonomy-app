...
export default function SelfMonitoringParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Self-Monitoring Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Self-Monitoring Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Self-Monitoring Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Self-Monitoring Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Self-Monitoring Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Self-Monitoring Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Self-Monitoring analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Self-Monitoring Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Self-Monitoring Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Self-Monitoring Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Self-Monitoring Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Self-Monitoring Parsed content length:', parsedContent.length);
    console.log('🔍 Self-Monitoring Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Self-Monitoring Parent Guide</ThemedText>
        <ThemedText style={styles.moduleSubtitle}>
          How to help your child develop self-assessment skills
        </ThemedText>
      </ThemedView>
      ...
      <SurveyButton 
        moduleId="self-monitoring-parent"
        moduleName="Self-Monitoring Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
