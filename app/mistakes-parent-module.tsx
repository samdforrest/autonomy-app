...
export default function MistakesParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Mistakes Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Mistakes Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Mistakes Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Mistakes Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Mistakes Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Mistakes Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Mistakes analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Mistakes Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Mistakes Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Mistakes Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Mistakes Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Mistakes Parsed content length:', parsedContent.length);
    console.log('🔍 Mistakes Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Mistakes Parent Guide</ThemedText>
        <ThemedText style={styles.moduleSubtitle}>
          Supporting your child through errors and setbacks
        </ThemedText>
      </ThemedView>
      ...
      <SurveyButton 
        moduleId="mistakes-parent"
        moduleName="Mistakes Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
