...
export default function RegulationParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Self-Regulation Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Self-Regulation Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Self-Regulation Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Self-Regulation Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Self-Regulation Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Self-Regulation Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Self-Regulation analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Self-Regulation Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Self-Regulation Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Self-Regulation Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Self-Regulation Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Self-Regulation Parsed content length:', parsedContent.length);
    console.log('🔍 Self-Regulation Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Self-Regulation Parent Guide</ThemedText>
      ...
      <SurveyButton 
        moduleId="regulation-parent"
        moduleName="Self-Regulation Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
