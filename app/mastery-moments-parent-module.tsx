...
export default function MasteryMomentsParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Mastery Moments Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Mastery Moments Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Mastery Moments Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Mastery Moments Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Mastery Moments Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Mastery Moments Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Mastery Moments analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Mastery Moments Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Mastery Moments Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Mastery Moments Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Mastery Moments Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Mastery Moments Parsed content length:', parsedContent.length);
    console.log('🔍 Mastery Moments Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Mastery Moments Parent Guide</ThemedText>
      ...
      <SurveyButton 
        moduleId="mastery-moments-parent"
        moduleName="Mastery Moments Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
