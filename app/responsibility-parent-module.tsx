...
// No sections interface needed - displaying content directly

export default function ResponsibilityParentModuleScreen() {
  const [revealedAnswers, setRevealedAnswers] = useState<Set<string>>(new Set());
  
  // Google Docs content for the currently active section
  const { content, loading, error, refetch } = useGoogleDocsContent(
    DOCUMENT_REFS.MAIN_DOCUMENT,
    'raw',
    { tab: 'Responsibility Parent How To' }
  );
  ...
  React.useEffect(() => {
    if (content) {
      ...
      if (content.contentBlocks) {
        console.log('📄 Content blocks:', content.contentBlocks);
  ...
  // Filter content based on current section
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            if (whyMatch) {
              console.log('📍 Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Final parsed bubbles:', parsedBubbles);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Responsibility Parent Guide</ThemedText>
        <ThemedText style={styles.moduleSubtitle}>
          How to guide your child's responsibility development
        </ThemedText>
      </ThemedView>
      ...
      {/* Survey Button */}
      <SurveyButton 
        moduleId="responsibility-parent"
        moduleName="Responsibility Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
