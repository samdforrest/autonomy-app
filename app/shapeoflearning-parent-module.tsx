...
export default function ShapeofLearningParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Shape of Learning Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Shape of Learning Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Shape of Learning Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Shape of Learning Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Shape of Learning Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Shape of Learning Parent Error:', error);
    }
  }, [content, error]);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Shape of Learning Parent Guide</ThemedText>
      ...
      <SurveyButton 
        moduleId="shapeoflearning-parent"
        moduleName="Shape of Learning Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
