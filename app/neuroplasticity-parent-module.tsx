...
export default function NeuroplasticityParentModuleScreen() {
  ...
  const { content, loading, error, refetch } = useGoogleDocsContent(
    ...
    'raw',
    { tab: 'Neuroplasticity Parent How To' }
  );

  // Debug: Log content structure
  React.useEffect(() => {
    if (content) {
      console.log('📄 Neuroplasticity Parent Content loaded:', {
      ...
      if (content.contentBlocks) {
        console.log('📄 Neuroplasticity Content blocks:', content.contentBlocks);
        content.contentBlocks.forEach((block: any, index: number) => {
          console.log(`Neuroplasticity Block ${index}:`, {
            id: block.id,
            header: block.header,
          ...
          if (block.content) {
            block.content.forEach((item: any, itemIndex: number) => {
              console.log(`  Neuroplasticity Content item ${itemIndex}:`, {
                type: item.type,
                text: item.text,
                fullItem: item
              });
    ...
    if (error) {
      console.error('❌ Neuroplasticity Parent Error:', error);
    }
  }, [content, error]);
  ...
  const parseWhyHowWhatContent = () => {
      ...
      if (block.content && block.content.length > 0) {
        ...
        block.content.forEach((item: any, itemIndex: number) => {
          if (item.type === 'text' && item.text) {
            ...
            console.log(`🔍 Neuroplasticity analyzing text: "${text}"`);
            ...
            if (whyMatch) {
              console.log('📍 Neuroplasticity Found Why section:', whyMatch[1]);
              parsedBubbles.push({
            ...
            if (howMatch) {
              console.log('📍 Neuroplasticity Found How section:', howMatch[1]);
              parsedBubbles.push({
            ...
            if (whatMatch) {
              console.log('📍 Neuroplasticity Found What section:', whatMatch[1]);
              parsedBubbles.push({
    ...
    console.log('🎯 Neuroplasticity Final parsed bubbles:', parsedBubbles);
  ...
  const renderContent = () => {
    ...
    console.log('🔍 Neuroplasticity Parsed content length:', parsedContent.length);
    console.log('🔍 Neuroplasticity Parsed content:', parsedContent);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <ThemedView style={styles.header}>
        <ThemedText style={styles.moduleTitle}>Neuroplasticity Parent Guide</ThemedText>
        <ThemedText style={styles.moduleSubtitle}>
          Explaining brain growth and development to your child
        </ThemedText>
      </ThemedView>
      ...
      <SurveyButton 
        moduleId="neuroplasticity-parent"
        moduleName="Neuroplasticity Parent Guide"
        dayNumber={1}
      />
    </ScrollView>
  );
